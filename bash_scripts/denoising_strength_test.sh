...
# TODO:
# predefined range of paramaeter list for denoising strength test
# fix tone settings
# run inference loop
...
# denoising strengths and alpha
denoising_strength_start=(0.2 0.4 0.6 0.8)
denoising_strenght_end = (0.0)
alpha=(0.0)
...
run_inference() {
    ...
    local mask_path="$2"
    local description="$3"
    local denoising_strength_start="$4"
    local denoising_strength_end="$5"
    local alpha="$6"
    ...
    curl --fail --silent --show-error -X POST "$API_URL" \
        ...
        -d "{
            \"start\": {
                \"prompt\": \"\",
                \"seed\": 42,
                \"denoising\": $denoising_strength_start,
                \"guidance\": $guidance_start
            },
            \"end\": {
                \"prompt\": \"\",
                \"seed\": 123,
                \"denoising\": $denoising_strength_end,
...
for i in "${!SEED_IMAGES[@]}"; do
    for j in "${!MASK_IMAGES[@]}"; do
        ...
        run_inference "$seed_path" "$mask_path" "Seed_${seed_num}_$(basename "$seed_path") vs Mask_${mask_num}_$(basename "$mask_path")"\
         "${denoising_strength_start[$i]}" "${denoising_strength_end[$i]}" "${alpha[$i]}"
    done
done
