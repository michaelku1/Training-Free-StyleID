...
class StyleIDStableAudioInference:
    """
    Complete inference pipeline for StyleID-enhanced Stable Audio Open v1.0 audio style transfer.
    """
    ...
    def __init__(
        ...
        """
        Initialize the StyleID StableAudio inference pipeline.
        
        Args:
            model_path: Path to the StableAudio model checkpoint
        ...
        self.styleid_params = {
            ...
            'injection_layers': [0, 1]  # Attention layers for injection (adjusted to match actual model)
        }
        if styleid_params:
        ...
        if self.enable_memory_optimization:
            # Note: StableAudio uses different component names than Riffusion
            if hasattr(self.pipeline, 'diffusion_transformer'):
                self.pipeline.diffusion_transformer.eval()
            if hasattr(self.pipeline, 'audio_vae'):
                self.pipeline.audio_vae.eval()
            if hasattr(self.pipeline, 'text_encoder'):
                self.pipeline.text_encoder.eval()
            
            # Enable gradient checkpointing for diffusion transformer
    ...
    def spectrogram_to_audio(self, spectrogram_image: Image.Image, output_path: str):
        ...
        # For StableAudio, this would convert the spectrogram back to audio
        # This is a placeholder implementation
        # In practice, this would use StableAudio's audio conversion utilities
        
        # Create a simple audio segment for testing
        audio_segment = pydub.AudioSegment.silent(duration=5000)  # 5 seconds of silence
    ...
    def style_transfer(
        ...
        """
        print("=" * 60)
        print("STYLEID STABLEAUDIO AUDIO STYLE TRANSFER")
        print("=" * 60)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="StyleID StableAudio Audio Style Transfer")
    
    # Model parameters
    parser.add_argument("--model_path", default="stabilityai/stable-audio-open-1.0", 
                       help="Path to StableAudio model checkpoint")
